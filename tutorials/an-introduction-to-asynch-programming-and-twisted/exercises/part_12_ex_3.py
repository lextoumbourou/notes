...
class CoolProtocol(NetstringReceiver):
...
class TransformFactory(ServerFactory):
    ...
    def xform_halve(self, poem):
        return self.service.halve(poem)


class CoolFactory(TransformFactory):
...
def main():
    ...
    cool_factory = CoolFactory(service)
    ...
    port_2 = reactor.listenTCP(0, cool_factory,
                             interface=options.iface)


    print 'Serving transforms on %s.' % (port.getHost(),)
    print 'Serving transforms on %s.' % (port_2.getHost(),)
