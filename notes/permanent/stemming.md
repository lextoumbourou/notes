...
**Stemming** is a text normalisation technique in [Natural Language Processing](natural-language-processing.md) that is used to reduce a word to its common root form. It uses a crude, rule-based approach which typically chops of common suffixes like `-ing`, `-eg`, `ly`. It is fast, but unlike [Lemmatisation](lemmatisation.md) it produces non-words: `argue` -> `argu`.