...
A function is a rule that assigns to each element a unique element.

Functions are denoted by letters f, g, h, i, etc.

If f is a function from X to Y, we write f : X -> Y. The set X is the domain of the function f and Y the codomain of f, denoted by dom(f) and codom(f).
