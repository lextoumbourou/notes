...
- Introduced a high-fidelity universal [Audio Compression Model](Audio%20Compression%20Model) called [Improved RVQGAN](Improved%20RVQGAN)
    - Compress 44.1 KHz audio into [Discrete Codes](Discrete%20Codes) at 8 kbps bitrate (~90x compession) with minimal loss in quality and fewer artifacts.
    - Outperforms state-of-the-art methods by large margin even at lower bitrates (high compression)
    - Capable of handling speech, music, environmental sounds, different sampling rates and audio encoding formats.
    - Evaluations:
        - quantitative metrics.
        - qualitative listening tests.
* Identify critical issue in existing models where they don't fully utilitse bandwidth due to "codebook collapse" (where a fraction of the codes are unused)
    * Fix it with "codebook learning techniques"
    
* Identify side-effect of quanitizer dropout
    * Technique designed to allow a single model to support variable bitrates
        * Hurts full-bandwidth audio quality and propose a solution to mitigate.

* Make impactful design changes to existing neural audio codecs by adding periodic inductive biases
...
- Generative modelling of high-resolution audio is difficult because:
    - high dimensionality (~44,100 samples per second of audio)
        - See [An unconditional end-to-end neural audio generation model](An%20unconditional%20end-to-end%20neural%20audio%20generation%20model)
...
- Common mitigations:
    - audio generation is typically divided into two stages:
        - 1. predicting audio conditioned on some intermediate representation such as mel-spectrograms see:
            - [An unconditional end-to-end neural audio generation model](An%20unconditional%20end-to-end%20neural%20audio%20generation%20model)
...
- An [Audio Compression Model](Audio%20Compression%20Model) needs to satisfy the following properties:
    ...
    - Achieve high level of compression along with temporal downscaling to learn a compact representation that discards low-level imperceptible details while preserving high-level structure:
        ...
        - [Generating diverse high-fidelity images with vq-vae-2](Generating%20diverse%20high-fidelity%20images%20with%20vq-vae-2)
...
- [BigVGAN](../../../../permanent/BigVGAN.md)
    - Improve HifiGAN recipe by introducing a periodic inductive bias using the [Snake Activation Function](../../permanent/snake-activation-function.md)
        - [Neural networks fail to learn periodic functions and how to fix it](Neural%20networks%20fail%20to%20learn%20periodic%20functions%20and%20how%20to%20fix%20it)
...
- Use a fully causal convolutional encoder and decoder network, and perform [Residual Vector Quantization](Residual%20Vector%20Quantization)
- The model is trained using the VQ-GAN formulation by adding adversarial and feature matching losses along with the multi-scale spectral reconstruction loss
    - See [Taming transformers for high-resolution image synthesis](Taming%20transformers%20for%20high-resolution%20image%20synthesis)
...
- [Residual Vector Quantization](Residual%20Vector%20Quantization)
...
- Obtain a stable training recipe using best practices for adversarial and perceptual loss design, with fixed loss weights and without needing a sophistacted loss balancer.
...
Language modeling of natural signals : Neural language models have demonstrated great success in diverse tasks such as open-ended text generation [6] with in-context learning capabilities.
...
Mapping usually learned with [VQ-GAN](VQ-GAN), followed by training autoregressive Transformer on discrete tokens.
- [Taming transformers for high-resolution image synthesis](Taming%20transformers%20for%20high-resolution%20image%20synthesis).
...
Built on framework of [VQ-GAN](VQ-GAN) models
...
- And introduced to audio domain in BigVGAN neural vocoding model.
    -  See [[Bigvgan: A universal neural vocoder with large-scale training]]
