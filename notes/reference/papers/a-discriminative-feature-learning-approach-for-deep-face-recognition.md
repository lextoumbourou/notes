...
This paper proposes a new supervision signal called [Center Loss](../../permanent/center-loss.md). Center Loss simultaneously learns a center for each class and penalizes the distance between features and their class centers.
...
Since [Stochastic Gradient Descent](../../permanent/stochastic-gradient-descent.md) relies on mini-batches, you cannot represent the global distribution in every step.

Alternatives proposed include [Contrastive Loss](../../permanent/contrastive-loss.md) (training using pairs) and Triplet Loss (training using triplets). However, they rely on Hard Negative Mining for efficiency, which adds complexity to the training pipeline.

This paper proposes [Center Loss](../../permanent/center-loss.md). They add a center for each class, a vector of the same dimension as the input feature embedding.
...
By combining Center Loss with [Softmax Loss](../../permanent/softmax-loss.md) to jointly supervise the learning of CNNs, the authors show that they can enhance the discriminative power of features for face recognition problems.
