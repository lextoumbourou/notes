...
The two contributions from this paper are:

* [CLAP](../../permanent/clap.md)
    * a training method and a particular architecture for learning a text/audio latent space.
    * This is useful for text to audio queries, and also for downstream tasks like audio classification.
    
* [Laion Audio 630K](../../permanent/dataset-laion-audio-630K.md)
    * A large text/audio dataset.
## Abstract

- [Contrastive Learning](../../permanent/contrastive-learning.md) frequently used in multi-modal [Representation Learning](../../permanent/representation-learning.md)
- Goal of paper: develop audio representation by combining audio data with text descriptions.
    - Release dataset [Laion Audio 630K](../../permanent/dataset-laion-audio-630K.md)
        - 633,526 audio-text pairs from different data sources.
    - Create pipeline of Contrastive Language-Audio Pre-training: [CLAP](../../permanent/clap.md) 
        - Experiment with different audio encoders and text encoders, and open-source a proposed model.
            - See https://github.com/LAION-AI/CLAP
    - Incorporates [Feature-Fusion](Feature-Fusion) mechanism
    - Add [keyword-to-caption augmentation](keyword-to-caption%20augmentation) to improve performance.
- Tasks evaluated:
    - [Text-to-Audio Retrieval](Text-to-Audio%20Retrieval)
...
* Most audio tasks in ML require annotated data.
* Unsupervised [Representation Learning](../../permanent/representation-learning.md) for audio is an ongoing challenge.
* [Contrastive Learning](../../permanent/contrastive-learning.md) a widely used solution for training on noisy internet data.
* A particular implementation: [CLIP](../../permanent/contrastive-language-image-pretraining.md)
    * Learns relationship between text and image by projecting them into a shared [Latent Space](../../../../permanent/latent-space.md).
    * Use ground-truth image-text pair as positive sample, and "left" as negative.
    * Not constrained by data annotation, and has robustness in out-of-domain tests using [ImageNet](../../../../permanent/ImageNet.md).
    * Useful for downstream tasks like text-to-image retrieval and text-guided captioning.
    * Observation: like vision, audio and language has overlapping info.
    * Text descriptions of event can be be mapped to corresponding audio.
    * Text descriptions share a similar meaning that could be learned together with the related audio to form an audio [Representation](Representation) of cross-modal information.
...
* Contrastive language-audio pre-training has been covered before:
    ...
    * [Audio Retrieval with WavText5K and CLAP Training](../../permanent/audio-retrieval-with-wavtext5k-and-clap-training.md)
        * Adds [HTS-AT](../../permanent/htsat.md) and [RoBERTa](RoBERTa) into encoder list to enhance performance.
...
* This paper makes the following contributions based on these concerns:
    * Release [Laion Audio 630K](../../permanent/dataset-laion-audio-630K.md)
        * largest public audio caption dataset of 633,526 audio-text pairs.
...
* [Laion Audio 630K](../../permanent/dataset-laion-audio-630K.md)
    ...
    * Contains:
        ...
        * natural sounds
        * audio effects
    * 8 data sources from publicly available websites.
* Training datasets
...
* [CLAP](../../permanent/clap.md)
    ...
    * Like [CLIP](../../permanent/contrastive-language-image-pretraining.md) has 2 encoders:
        ...
        * Where audio/text projectiction layer is a 2-layer multilayer perceptron (MLP) with ReLU as activication function to map encoder outputs into sampe dimensions  (i.e., Eai, Et i ∈ RD)
    * Trained with contrastive learning paradigm between the audio and text embedddings in pair, following the same loss function in [1]:
...
* Zero-shot audio classification
    ...
    * One advantage of using the contrastive language-audio pretraining is that the categories of audio are unrestricted (i.e., zero-shot) since the model can convert the classification task into the text-to-audio retrieval task.
* Supervised Audio Classification
...
* [Feature-Fusion](Feature-Fusion)
    * Problem: audio is natural variable length, until image data which we can resize to "unified resolution" (ie 224x224).
        * Possible solution: average per-frame or per-chunk audio embeddings as output.
            * Computationally inefficient on long audio.
    * CLAP approach: Feature Fusion (see Fig. 1 above)
        ...
        * For an audio in T seconds and a fixed chunk duration d = 10 seconds:
            * $T < d$: repeat the input, then pad with zero values.
                * For example, 3-second input will be repeated 3 x 3 = 9 second and padded with 1-second zero values.
            * $T > d$:
...
* [[Keyword-to-Caption Augmentation]]
    ...
    * Also do a [[De-bias]] the output sentence in post-processing.
...
- 3 experiments conducted:
    ...
    - Train model on various dataset size, with feature fusion and keyword-to-caption augmentation to verify efficacy of proposed methods.
     ...
     - Lastly, we use the best model to conduct zero-shot and supervised audio classification experiments to evaluate the generalization ability to the downstream tasks.
 
### 4.1. Hyperparameters and Training Details

- Use [[AudioCaps]], [[Clotho]], [[LAIONAudio-630K]], also [[AudioSet]] by keyword-to-caption augmentation, to train our model.
- For the audio data, we use 10-second input length, 480 hop size, 1024 window size, 64 mel-bins to compute STFTs and mel-spectrograms
 * As the result, each input sent to the audio encoder is of the shape (T = 1024, F = 64).
 * For the text data, we tokenize the text with a maximum token length of 77.
...
* In this experiment, train on [AudioCaps](AudioCaps) and [[Clotho]] datasets, and report the best mAP@10 on audio-to-text (A→T) and text-to-audio (T→A) perspectives
* Results:
...
* Dataset Scale
    * Consequently, we apply HTSAT-RoBERTa as our best model setting to conduct the text-to-audio retrieval experiments as a comprehensive evaluation in Table 3.
    * We adopt the same metrics in [7, 8] to compute recall scores at different ranks in this task.
...
We classify audio by performing audio-to-text retrieval with each text corresponds to the text prompt converted from class label via “This a sound of label.”. We noticed a dataset overlap between our training data and the zero-shot dataset we are evaluating on. We excluded all the overlap samples and perform zero-shot evaluation on the whole remaining dataset

Supervised Audio Classification We perform supervised audio
classification by fine-tuning the audio encoder on FSD50K [30] and
VGGSounddatasets. We do not conduct this experiment on ESC50 and Urbansound8K because the potential data leakage issue in
...
of zero-shot audio classification across all three datasets, demonstrating the high generalization ability of our model to unseen data.
...
Feature fusion not only enables the model
to handle variable-length input, but also achieves better performance
than previous models

Our best supervised audio classification result
outperforms the current state-of-the-art on VGGSound dataset and
is close to state-of-the-art on FSD50K dataset.

The results verify
that the proposed model also learns efficient audio representation
during contrastive learning paradigm.

## 5. Future Work

* Collecting even larger dataset.
* Apply representations into more downstream tasks like audio synthesis and separation.