...
Then you need to construct a query where for some child category, you want to fetch the entire category hierarchy. For example, starting at **Nike Air** walk up the tree to find all parent categories (**Sports** -> **Shoes**).

I recently was tasked with this exact problem. As usual, the [PostGres docs](http://www.postgresql.org/docs/current/static/queries-with.html) are very comprehensive, however, they can be a little daunting when you're getting started - especially if writing SQL isn't something you're doing every day. Since the difficulties of understanding it are fresh on my mind, I figured I'm in a good position to help other people get started with them. 
...
We start by selecting our parent from the base level, and this time join anycategory with a parent id that matches our query.
